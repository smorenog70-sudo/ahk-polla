...
export default function Standings() {
  ...
  const shareTable = async () => {
    ...
    try {
      ...
      const file = new File([blob], 'tabla-polla-ahk.png', { type: 'image/png' })
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Tabla Polla AHK',
          text: `Así va la Polla AHK 🏆 · ${filterLabel(filter, bracketTeamsMap)}`,
        })
        ...
        a.href = url
        a.download = 'tabla-polla-ahk.png'
      ...
      <div className="card">
        ...
        <p className="text-xs text-ink-300">
          {filter === 'total'
            ...
            : `Solo los puntos de: ${filterLabel(filter, bracketTeamsMap)}.`}
        </p>
      </div>
                ...
                >
                  ...
                  <td className="py-2 px-2">
                    ...
                    <div className="text-xs text-ink-500">
                      {r.is_admin && <span className="text-brand-500">admin</span>}
                    </div>
                  </td>
...
// ===================================================================
// Generar imagen de la tabla para compartir (estilo Polla AHK azul)
// ===================================================================
async function renderTableImage(rows, filter, bracketTeams = {}) {
  ...
  ctx.fillText('POLLA AHK', W / 2, 215)
  ctx.fillStyle = '#cbd5e1'
  ctx.font = '26px -apple-system, system-ui, sans-serif'
  const sub = filter === 'total' ? 'Tabla acumulada' : `Tabla · ${filterLabel(filter, bracketTeams)}`
  ...
  ctx.fillText('🏆 Polla AHK · Mundial 2026', W / 2, H - 28)
  ...
  return (
    <div className="card">
      <div className="flex items-center gap-3 mb-3 text-[10px]">
        <span className="flex items-center gap-1"><span className="inline-block w-3 h-2 rounded-sm" style={{ background: '#2E5DA8' }} /> Partidos</span>
          ...
          return (
            <div key={r.id} className={`flex items-center gap-2 ${isMe ? 'bg-brand-900/30 -mx-1 px-1 rounded' : ''}`}>
              ...
              <div className="flex-1 h-5 bg-ink-900/40 rounded overflow-hidden flex">
                <div className="h-full" style={{ width: `${matchW}%`, background: '#2E5DA8' }} />
...
function RaceView({ raceData, userId }) {
  ...
  const palette = ['#2E5DA8', '#fbbf24', '#38bdf8', '#a78bfa', '#34d399']
  const lineFor = (s, idx) => {
