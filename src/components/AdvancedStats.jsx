  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
        {/* línea de mi puesto */}
        <polyline points={points} fill="none" stroke="var(--brand-500, #2E5DA8)" strokeWidth="2" />
        {series.map((s, i) => (
          <circle key={i} cx={x(i)} cy={y(s.rank)} r="2.5" fill="var(--brand-400, #4d7bc0)" />
        ))}
        ...
        <text x={x(series.length - 1)} y={y(last.rank) - 6} fontSize="10" fill="#4d7bc0" textAnchor="middle" fontWeight="bold">{last.rank}º</text>
      </svg>
